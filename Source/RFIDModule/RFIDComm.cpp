...
CRFIDComm::CRFIDComm(CLogModel *model, QObject *parent)
    :QObject(parent)
{
    m_pLogModel = model;
}
...
bool CRFIDComm::OpenSerialCom()
{
    ...
    QString comName = reverse_mapComName[m_pSettingData->rfidComName()];
    QString strBaudRate = reverse_mapComName[m_pSettingData->rfidBaudRate()];
